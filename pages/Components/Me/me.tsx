...
interface MeProps {
	...
	onResize: (scrollTop: number) => void;
}

const Me = forwardRef<HTMLDivElement, MeProps>(({collapseHeader, scrollRef, onResize}, headerRef) => {
	...
	const update = useRef((ref: React.RefObject<HTMLDivElement>) => {
		setHeight(() => ref.current?.getBoundingClientRect().height || 0);
	});

	useEffect(() => {
		const updater = () => update.current(skillsWrapRef);
		window.addEventListener('resize', updater);
		updater();
		onResize?.(scrollRef.current.scrollTop);
		return () => window.removeEventListener('resize',updater);
	});

	const renderInner = (collapse: boolean) => {
		return (
			<div
				className={styles.me + (collapse ? ' ' + styles._collapse : '')}
				ref={!collapse ? headerRef : undefined}
			>
				...
				<div
					...
					style={{
						height: (!showSkills || collapse) ? 0 : `calc(${height}px + 1.15em)`,
					}}
				>
					<div
						ref={skillsWrapRef}
	...
	return (
		<>
			{renderInner(false)}
			{collapseHeader && renderInner(true)}
		</>
	);
});
